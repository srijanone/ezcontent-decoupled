...
export default props => {
  ...
  const block = (
    <>
      {
        content.map((item, i) => {
          const BackgroundImgClass = item?.layoutBuilderBackground;
          return (
          <div style={{ backgroundImage: `url(${process.env.API_HOST}${item?.layoutBuilderBackground?.media })` , backgroundColor:BackgroundImgClass?.color ,backgroundPosition : BackgroundImgClass?.position }} className={item.rowCssClass ? (landingPageCheck) ? `row ${item.rowCssClass} ${item?.customClasses}` : item.rowCssClass : ''}
            id={item.customId !== null ? item?.customId : null} data={item.customDataAttributes !== null ? item?.customDataAttributes : null} key={`block_${i}`}>
            {
              sortByWeight(item.components).map((compVal, j) => {

                let layoutId = item.layout_id;
                let componentAttributes = item?.layoutBuilderComponent[j]?.additional?.component_attributes;
                if (landingPageCheck) {
                  let dynamicAttributes = componentAttributes?.block_attributes?.data.split("\n").reduce(function(obj, str, index) {
                 ...
                 let dynamicStyle = componentAttributes?.block_attributes?.style.split(" ").reduce(function(obj, str, index) {
                    let strParts = str.split(":");
                    if (strParts[0] && strParts[1]) {
                  ...
                  return (
                    <div className={
                      ` ${item.layout_id == "full_width_content"
                        // ? "container"
                        ? "col-12"
                        : 'col-' + colArr[item.layout_id]} ${componentAttributes?.block_attributes?.class ? componentAttributes.block_attributes.class : ""}`
                    } id={componentAttributes?.block_attributes?.id ? componentAttributes.block_attributes.id : null} {...dynamicAttributes}
                      style={{...dynamicStyle}}
                      key={`block__${i}${j}`}>
                      {
                        compVal.map((comp, k) => {
                          comp['componentAttributes'] = componentAttributes;
